#!/usr/bin/env bash
# Habilita http://scriptorium (puerto 80) en este Mac. Ejecutar UNA vez con
# sudo; install.sh ya deja corriendo el servidor Caddy de usuario en :8080.
#
#   sudo bash macos/scriptorium-root-setup.sh
#
# Hace tres cosas a nivel de sistema (por eso requiere root); todas idempotentes:
#   1. Resuelve el nombre `scriptorium` a 127.0.0.1 en /etc/hosts.
...
# 2. Regla pf 80 -> 8080 ───────────────────────────────────────────────────────
install -m 644 "$DOTFILES/macos/scriptorium.pf.anchor" /etc/pf.anchors/scriptorium
install -m 644 "$DOTFILES/macos/scriptorium-pf.conf"   /etc/pf-scriptorium.conf
echo "✓ Instalada regla pf (puerto 80 -> 8080)"

# 3. LaunchDaemon que recarga pf al arrancar ───────────────────────────────────
daemon=/Library/LaunchDaemons/com.fran.scriptorium-pf.plist
install -m 644 -o root -g wheel "$DOTFILES/macos/com.fran.scriptorium-pf.plist" "$daemon"
...
pfctl -ef /etc/pf-scriptorium.conf 2>/dev/null || true
