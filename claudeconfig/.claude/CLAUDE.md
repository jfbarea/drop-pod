...
- **Excepción por repo.** El hook exime los repositorios listados en `CLAUDE_UNPROTECTED_REPOS`, que por defecto es `jfbarea/drop-pod`. Los identifica por el `owner/repo` del remote `origin`, no por la ruta, así que la excepción viaja con el clon. En un repo exento puedes commitear y pushear a `main` sin preguntar. La excepción **no** se aplica cuando el comando expande variables o subshells: ahí el hook sigue bloqueando por precaución, para que un `-C` con variable no se atribuya al repo equivocado.
